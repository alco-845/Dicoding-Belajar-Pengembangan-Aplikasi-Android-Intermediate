...
class AddViewModel(private val repository: AppRepository) : ViewModel() {
    ...
    fun addStory(token: String, description: RequestBody, file: MultipartBody.Part, lat: RequestBody?, lon: RequestBody?) {
        _isLoading.value = true
        val response: Call<DataResponse> = repository.addStory(token, description, file, lat, lon)
